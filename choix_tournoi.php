...
<head>
    ...
    <title>SGTF - Choix Tournoi</title>
...
<body>
    ...
    <div class="container">
	 ...
	 <?php
		//insertion du formulaire du joueur dans la base de données 
		$nom_joueur=$_POST['nom_joueur'];
		$prenom_joueur=$_POST['prenom_joueur'];
		$mail_joueur=$_POST['mail_joueur'];
		$tel_joueur=$_POST['tel_joueur'];
		$req_insert_joueur = $db->prepare('INSERT INTO joueur(nom_joueur, prenom_joueur, mail_joueur, tel_joueur) VALUES(:nom_joueur, :prenom_joueur, :mail_joueur, :tel_joueur)');
		$req_insert_joueur->execute(array(
			'nom_joueur' => $nom_joueur,
			'prenom_joueur' => $prenom_joueur,
			'mail_joueur' => $mail_joueur,
			'tel_joueur' => $tel_joueur,
		));
	?>
	
	<?php 
		//on recupere l'id du nouveau joueur dans la base de données
		$req=$db->prepare('select id_joueur from joueur where nom_joueur=:nom_joueur and mail_joueur=:mail_joueur');
			$req->execute(array(
			'nom_joueur'=>$nom_joueur,
			'mail_joueur'=>$mail_joueur
			));
			$ligne=$req->fetch();
			$id_joueur=$ligne['id_joueur'];
			
	?>
			   <!-- on affiche les tournois disponibles à l'inscription sous forme de tableau -->
	<form method="post" action="inscription.php">
		<input type="hidden" name="id_joueur" value="<?php echo $id_joueur ?>" /> <!-- on crée un input caché afin de récupérer l'id du joueur et de le transmettre à la page suivante -->
			<table id="tab" style ="text-align:center">
				<caption class="text-center policy2">Voici les tournois disponibles :</caption>
					<colgroup>
					...
					<tbody>
					<?php //requête pour récuperer les données des tournois
					...
					?>
							<tr>
								<td><input type="radio" name="choix_id_tournoi" value="<?php echo $donnees['id_tournoi']; ?>" required /> </td> <!-- on crée une checkbox pour que le joueur sélectionne le tournoi auquel il veut s'inscrire -->
